...
export function SettingsCategoriesScreen() {
  ...
  const handleSubmit = useCallback(
    async (
      data: Omit<Category, "id" | "createdAt" | "updatedAt" | "householdId">,
    ) => {
      if (editing) {
                          ...
                          {item.subCategories &&
                            item.subCategories.length > 0 && (
                              <Text style={styles.rowSub}>
                                {item.subCategories.length} subcategor
                                {item.subCategories.length === 1 ? "y" : "ies"}
                              </Text>
                            )}
                        </View>
...
const styles = StyleSheet.create({
  ...
  separator: {
    ...
    marginLeft: SPACING.lg + 18 + SPACING.md, // indent past swatch
  },
  fab: {
    position: "absolute",
    bottom: 100,
